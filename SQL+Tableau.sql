...
-- Percentage of Male and Female Employees over the years
...
-- Querying employees between 2 period and indicating their active status using CASE statement
SELECT d.dept_name, ee.gender, dm.emp_no, dm.from_date, dm.to_date, e.calendar_year,
CASE
WHEN YEAR(dm.to_date) >= e.calendar_year AN	D YEAR(dm.from_date) <= e.calendar_year THEN 1
ELSE 0
END AS active
...
-- Comparing average male and female salary in the company till 2002 with filters
...
-- Stored procedure to obtain average male and female salaries within certain range
